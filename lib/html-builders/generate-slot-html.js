...
exports.generateSlotHtml = (slotConfig) => {
    ...
    const propsString = props.map(prop => {
        ...
        return ` ${name}=${knob ? render_knob_1.renderKnob(Object.assign(Object.assign({}, knob), { defaultValue: value }), knob.type) : convert_value_to_attribute_1.convertValueToAttribute(value)}`;
    }).join('');
