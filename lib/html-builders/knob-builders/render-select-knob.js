...
exports.renderSelectKnob = (config) => {
    ...
    return `\"\${select('${name}', ${options ? `${escape_object_for_template_literals_1.escapeObjectForTemplateLiterals(options)}, ` : ''}${escape_object_for_template_literals_1.escapeObjectForTemplateLiterals(defaultValue)}${groupId ? `, '${groupId}'` : ''})}\"`;
};
