...
exports.generateStoryHtml = (storyConfig) => {
    ...
    const configs = storyConfig.previewConfigs.map(config => {
        ...
        const propsString = Object.keys(props).map(propKey => {
            ...
            return ` ${propKey}=${isKnobProp ? render_knob_1.renderKnob(knobs[propKey].name, props[propKey], knobs[propKey].type) : convert_value_to_attribute_1.convertValueToAttribute(props[propKey])}`;
        }).join('');
