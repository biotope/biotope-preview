...
exports.runCreationOfStoriesFiles = async (globalResources) => {
    ...
    await Promise.all(importedConfigurations.map((config) => create_stories_file_1.createStoriesFileForConfig(config, globalResources)));
    await Promise.all(importedConfigurations.map((config) => create_docs_file_1.createDocsFileForConfig(config)));
};
