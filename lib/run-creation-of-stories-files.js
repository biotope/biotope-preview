...
const get_sub_files_1 = require("./file-handlers/get-sub-files");
...
exports.runCreationOfStoriesFiles = async () => {
    const subFolders = await get_sub_files_1.getSubFiles(componentsSrc, true, true);
    const filesPaths = await Promise.all(subFolders.map(subFolder => get_sub_files_1.getSubFiles(subFolder, true, false)));
