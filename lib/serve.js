...
(async () => {
    ...
    const componentsSrcDir = get_param_value_from_args_1.getParamValueFromArgs(userConfig, "componentsSrcDir") || 'src/components';
    const staticDir = get_param_value_from_args_1.getParamValueFromArgs(userConfig, "staticDir") || 'dist/resources/components';
    await run_creation_of_stories_files_1.runCreationOfStoriesFiles(componentsSrcDir);
    await run_storybook_1.runStorybook({ staticDir, mode: 'dev' });
