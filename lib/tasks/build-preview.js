...
async function buildPreview() {
    ...
    await run_creation_of_stories_files_1.runCreationOfStoriesFiles(globalConfig.globalResources).catch((err) => err);
    await create_theme_file_1.createThemeFile(globalConfig.theme).catch((err) => err);
    await run_storybook_1.runStorybook({ mode: 'static', staticDir: globalConfig.resourcesDir, outputDir: globalConfig.outputDir }).catch((err) => err);
}
